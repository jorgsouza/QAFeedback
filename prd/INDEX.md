...
Documentação de produto e planos de implementação vivem em **`prd/`**, numerados por ordem de arquivo (**PRD-001** … **PRD-012**). Cada pasta pode ter `prd.md` (especificação), `plan.md` (plano técnico / fases) ou vários arquivos (ex.: spec + anexos).
...
| **PRD-012** | Gravação contínua de viewport (WebM) para evidências QA | [prd.md](PRD-012-gravacao-viewport/prd.md), [plan.md](PRD-012-gravacao-viewport/plan.md) | Em implementação — branch `feature/prd-012-viewport-webm-recording` (offscreen + tabCapture + MediaRecorder + UI) |
