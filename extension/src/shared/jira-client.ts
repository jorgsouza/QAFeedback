...
/** Remove prefixo data URL, normaliza URL-safe, remove lixo e ajusta padding. */
function normalizeJiraAttachmentBase64(raw: string): string {
  let s = (raw ?? "").trim();
  ...
  if (pad === 2) s += "==";
    ...
    /** Comprimento inválido para base64 — último carácter órfão. */
    s = s.slice(0, -1);
  }
  return s;
}
...
/**
 * Decodifica base64 para bytes sem `atob` (evita falhas do motor com strings de vários MB em anexos WebM/HAR).
 */
function base64ToUint8Array(b64: string): Uint8Array {
  const n = normalizeJiraAttachmentBase64(b64);
...
export async function uploadJiraIssueAttachments(params: {
  ...
  for (const a of attachments) {
    ...
    const attachName = a.fileName?.trim() || "anexo";
    try {
      bytes = base64ToUint8Array(a.base64);
    } catch {
      return { ok: false, message: `Anexo «${attachName}»: dados base64 inválidos.` };
    }
