...
export async function uploadJiraIssueAttachments(params: {
  ...
  for (const a of attachments) {
    ...
    if (a.binary != null && a.binary.byteLength > 0) {
      const src = a.binary;
      bytes = new Uint8Array(src.byteLength);
      bytes.set(src);
    } else if (typeof a.base64 === "string" && a.base64.length > 0) {
      try {
      ...
      return { ok: false, message: `Anexo «${attachName}»: sem dados do ficheiro.` };
    }
